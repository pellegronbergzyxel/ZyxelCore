XmlPort 50013 "eCommerce Tax Doc. Import"
{
    // 001. 03-03-23 ZY-LD 000 - We have seen that the same VAT number has been present twice in the field.
    // 002. 03-03-23 ZY-LD 000 - We have seen orders where the same line has been copied several times, and gave a wrong invoice. This could be the reason.
    // 003. 08-06-23 ZY-LD 000 - New fields.
    // 004. 15-12-23 ZY-LD 000 - Donations is inserted.
    // 005. 22-07-24 ZY-LD 000 - So special characters can be imported.

    Caption = 'eCommerce Tax Doc. Import';
    Direction = Import;
    Format = VariableText;
    TextEncoding = UTF8;  // 22-07-24 ZY-LD 005
    RecordSeparator = '<LF>';
        ...
        textelement(Root)
        {
            tableelement(Integer; Integer)
            {
                ...
                begin
                    ...
                    if SetValues then
                        if not OrderPreviousCreated and AcceptVatNumber(SellerTaxRegistration, OrderDate, OrderID, VATInvoiceNumber, TransactTypeOption) then begin
    ...
    begin
        ...
        if recAmzOrderHead.FindSet(true) then
            repeat
                ...
                if not recAmzOrderHead.Modify(true) then;  // Document can be deleted during ValidateDocument.
    ...
    begin
        ...
        //>> 03-03-23 ZY-LD 002
        recAmzOrderHead.SetRange("Completely Imported", false);
        if recAmzOrderHead.Count > 0 then
            recAmzOrderHead.DeleteAll(true);
        //Error(Text002);
        recAmzOrderHead.SetRange("Completely Imported");
        //<< 03-03-23 ZY-LD 002
    end;
    ...
    var
        ...
        TaxReportingScheme: Text[250];
        TaxCollectionResponsibility: Text[250];
        ...
        Text002: label 'You can not import when there are orders which is not marked as completely imported.';
    ...
    begin
        ...
        TaxReportingScheme := Col19;
        TaxCollectionResponsibility := Col20;  // 08-06-23 ZY-LD 003
        TaxAddressRole := Col21;  // 08-06-23 ZY-LD 003
        ...
        //>> 03-03-23 ZY-LD 001
        if StrLen(VATInvoiceNumber) > 35 then
            if CopyStr(VATInvoiceNumber, 1, StrLen(VATInvoiceNumber) / 2) = CopyStr(VATInvoiceNumber, (StrLen(VATInvoiceNumber) / 2) + 1, StrLen(VATInvoiceNumber) / 2) then
                VATInvoiceNumber := CopyStr(VATInvoiceNumber, 1, StrLen(VATInvoiceNumber) / 2);
        //<< 03-03-23 ZY-LD 001
        InvoiceUrl := Col53;
    ...
    begin
        begin
            ...
            recAmzOrderHead.SetTaxValue(TaxType, TaxCalculationReasonCode, TaxAddressRole, BuyerTaxRegistrationType, TaxReportingScheme, TaxCollectionResponsibility);
            ...
            recAmzOrderHead.Validate(recAmzOrderHead."Invoice No.", VATInvoiceNumber);
            recAmzOrderHead.Validate(recAmzOrderHead."Ship To City", COPYSTR(ShipToCity, 1, MAXSTRLEN(recAmzOrderHead."Ship To City")));
            ...
            recAmzOrderHead.Validate(recAmzOrderHead."Import Source Type", recAmzOrderHead."import source type"::"Tax Library Document");
    ...
    begin
        ...
        Res1 := (recAmzOrderHead2.Get(TransactTypeOption, OrderID, VATInvoiceNumber) and recAmzOrderHead2."Completely Imported");
        Res2 := recAmzOrderArch.Get(TransactTypeOption, OrderID, VATInvoiceNumber);
        ...
        if ZGT.IsZComCompany and ZGT.IsRhq then
            ...
            if ZGT.IsZComCompany then begin
                ...
                if GuiAllowed then begin
                    if (pTransTypeOption = Ptranstypeoption::Order) and
                       (pVATNo in ['686640822', 'ATU76537534', 'ATU86640822', 'BE0691804097', 'CZ684340226', 'DE812743356', 'ESN2764659E', 'FR43834569063', 'IT00203809991', 'NL825611349B01', 'PL5263207801'])
                    then
                        rValue := false
                    else
                        if (pVATNo = '') and (pOrderDate < 20220601D) then
