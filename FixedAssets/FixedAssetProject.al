report 50110 "Fixed Asset Proj. Value Zyxel"
{//14-01-2026 BK #531798
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/FixedAssetProjectedValue.rdlc';
    ApplicationArea = FixedAssets;
    Caption = 'Fixed Asset Projected Value Zyxel';
    ...
    begin
        ...
        DeprBookText := Copystr(StrSubstNo('%1%2 %3', DeprBook.TableCaption(), ':', DeprBookCode), 1, 50);
        MakeGroupTotalText();
        ValidateDates();
        if PrintDetails then begin
            FANo := copystr("Fixed Asset".FieldCaption("No."), 1, 50);
            FADescription := copystr("Fixed Asset".FieldCaption(Description), 1, 100);
        end;
        ...
        if DeprBook."Use Custom 1 Depreciation" then begin
            ...
            DeprCustom1Text := copystr(StrSubstNo('%1 %2 %3', DeprText, '+', Custom1Text), 1, 50);
        end;
        SalesPriceFieldname := copystr(FADeprBook.FieldCaption("Projected Proceeds on Disposal"), 1, 80);
...
#pragma warning restore AA0470
    ...
    begin
        case GroupTotals of
            GroupTotals::"FA Class":
                GroupCodeName := copystr("Fixed Asset".FieldCaption("FA Class Code"), 1, 50);
            GroupTotals::"FA Subclass":
                GroupCodeName := copystr("Fixed Asset".FieldCaption("FA Subclass Code"), 1, 50);
            GroupTotals::"FA Location":
                GroupCodeName := copystr("Fixed Asset".FieldCaption("FA Location Code"), 1, 50);
            GroupTotals::"Main Asset":
                GroupCodeName := copystr("Fixed Asset".FieldCaption("Main Asset/Component"), 1, 50);
            GroupTotals::"Global Dimension 1":
                GroupCodeName := copystr("Fixed Asset".FieldCaption("Global Dimension 1 Code"), 1, 50);
            GroupTotals::"Global Dimension 2":
                GroupCodeName := copystr("Fixed Asset".FieldCaption("Global Dimension 2 Code"), 1, 50);
            GroupTotals::"FA Posting Group":
                GroupCodeName := copystr("Fixed Asset".FieldCaption("FA Posting Group"), 1, 50);
        end;
        if GroupCodeName <> '' then begin
            GroupCodeName2 := GroupCodeName;
            if GroupTotals = GroupTotals::"Main Asset" then
                GroupCodeName2 := copystr(StrSubstNo('%1', SelectStr(GroupTotals + 1, GroupCodeNameTxt)), 1, 50);
            GroupCodeName := copystr(StrSubstNo('%1%2 %3', GroupTotalsTxt, ':', GroupCodeName2), 1, 50);
        end;
    end;
            ...
            GroupTotals::"Main Asset":
                begin
                    ...
                    if "Fixed Asset"."Component of Main Asset" = '' then
                        GroupHeadLine := copystr(StrSubstNo('%1%2', GroupHeadLine, '*****'), 1, 50);
                end;
            GroupTotals::"Global Dimension 1":
