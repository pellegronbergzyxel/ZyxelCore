...
report 50019 "Calculate Inventory ZX"
{
    //Copy of Report 790
    // 001. 12-02-21 ZY-LD 000 - Update with warehouse quantity.
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            CalcFields = "Warehouse Inventory";
                ...
                begin
                    ...
                    if not SkipCycleSKU("Location Code", "Item No.", "Variant Code") then
                        if ByBin then begin
                                    ...
                                    if WhseEntry."Entry No." <> OldWhseEntry."Entry No." then begin
                                        ...
                                        repeat
                                            ...
                                            if not ItemBinLocationIsCalculated(WhseEntry."Bin Code") then begin
                                                ...
                                                UpdateBuffer(WhseEntry."Bin Code", WhseEntry."Qty. (Base)", false, 0);  // 12-02-21 ZY-LD 001
                                            end;
                                            WhseEntry.Find('+');
                            ...
                            //>> 12-02-21 ZY-LD 001
                            if CalculateWarehouseInventory then
                                UpdateBuffer(
                                  '',
                                  "Item Ledger Entry".Quantity,
                                  true,
                                  Item."Warehouse Inventory")
                            else  //<< 12-02-21 ZY-LD 001
                                UpdateBuffer(
                                  '',
                                  "Item Ledger Entry".Quantity,
                                  true,
                                  0);  // 12-02-21 ZY-LD 001
                end;
    ...
    var
        ...
        CalculateWarehouseInventory: Boolean;
    ...
    local procedure UpdateBuffer(BinCode: Code[20]; NewQuantity: Decimal; CalledFromItemLedgerEntry: Boolean; NewWhseQty: Decimal)
    var
        DimEntryNo: Integer;
    begin
        //UpgradeReady
        ...
        if RetrieveBuffer(BinCode, DimEntryNo) then begin
            ...
            TempQuantityOnHandBuffer."Warehouse Quantity" := NewWhseQty;
        ...
        if TempQuantityOnHandBuffer.FindSet() then begin
            repeat
                ...
                if (PosQty = 0) and (NegQty = 0) and not AdjustPosQty then
                    //>> 12-02-21 ZY-LD 001
                    if ShowOnlyDifference and (TempQuantityOnHandBuffer.Quantity <> TempQuantityOnHandBuffer."Warehouse Quantity") or
                (not ShowOnlyDifference)
            then
                        if CalculateWarehouseInventory then
                            InsertItemJnlLine(
                                TempQuantityOnHandBuffer."Item No.", TempQuantityOnHandBuffer."Variant Code", TempQuantityOnHandBuffer."Dimension Entry No.",
                                TempQuantityOnHandBuffer."Bin Code", TempQuantityOnHandBuffer.Quantity, TempQuantityOnHandBuffer."Warehouse Quantity")
                        else  //<< 12-02-21 ZY-LD 001
                            InsertItemJnlLine(
                                TempQuantityOnHandBuffer."Item No.", TempQuantityOnHandBuffer."Variant Code", TempQuantityOnHandBuffer."Dimension Entry No.",
    ...
    procedure InitializeRequestZyxel(NewShowOnlyDifference: Boolean)
    begin
        ShowOnlyDifference := NewShowOnlyDifference;
        CalculateWarehouseInventory := true;
    end;
}
