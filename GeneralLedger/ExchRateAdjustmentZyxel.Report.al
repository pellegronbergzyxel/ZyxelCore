report 50003 "Exch. Rate Adjustment Zyxel"
{
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    field(ExchangeRateDateReq; ExchRateDate)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'Exchange Rate Date';
                        ToolTip = 'Specifies the date on exhange rate is calculated from. We have to calculate exchange rate on ex. 01-03-21, but with the posting date 28-02-21.';
                    }
    ...
    var
        ...
        ExchRateAdjmtProcess: Codeunit "Exch. Rate Adjmt. Proc. Zyxel";
    begin
    ...
    procedure InitializeRequest(NewStartDate: Date; NewEndDate: Date; NewPostingDescription: Text[100]; NewPostingDate: Date; NewExchangeRateDate: Date)
    begin
        ...
        //>> 01-03-21 ZY-LD 002
        ExchRateDate := NewExchangeRateDate;
        IF ExchRateDate = 0D THEN
            ExchRateDate := PostingDate;
        //<< 01-03-21 ZY-LD 002
    end;

    procedure InitializeRequest2(NewStartDate: Date; NewEndDate: Date; NewPostingDescription: Text[100]; NewPostingDate: Date; NewPostingDocNo: Code[20]; NewAdjCustVendBank: Boolean; NewAdjGLAcc: Boolean; NewExchangeRateDate: Date)
    begin
        InitializeRequest(NewStartDate, NewEndDate, NewPostingDescription, NewPostingDate, NewExchangeRateDate);
    ...
    begin
        ...
        ExchRateAdjmtParameters2."Exchange Rate Date" := ExchRateDate;
    ...
    local procedure SetExchangeRateDate()
    begin
        //>> 01-03-21 ZY-LD 002
        ExchRateDate := PostingDate;
