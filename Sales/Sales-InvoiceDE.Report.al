Report 50010 "Sales - Invoice DE"
{
    // //PAB 040718 Ticket 2018070410000133 Ensure all date formats are the same
    // 001. 09-07-20 ZY-LD P0455 - Shipment Method caption is changed to Incoterms, and "Ship-to City is added".
    // 002. 11-12-20 ZY-LD 2020121010000091 - Incoterms city can be different.
    // 003. 23-03-21 ZY-LD 2021032310000054 - The "CanGrow" is set on the fields on CustAddr.
    // 004. 09-08-21 ZY-LD 2021070210000087 - Position on External document No.
    ...
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Sales - Invoice DE.rdlc';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            column(NormalizedShipmentDate; NormalizedShipmentDate)
            {
            }
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(ExternalDocumentHeader; "Sales Invoice Header"."External Document No.")
                    {
                    }
                        ...
                        begin
                            ...
                            if "Sales Invoice Line".Type in ["Sales Invoice Line".Type::"G/L Account", "Sales Invoice Line".Type::Item] then
                                ReportEvent.GetEUArticle(
                                  ...
                                  "Sales Invoice Header"."EU 3-Party Trade",  // 10-06-22 ZY-LD 006
                                  ArticleText);
            ...
            trigger OnAfterGetRecord()
            var
                LocalSalesInvoiceLine: Record "Sales Invoice Line";
            begin
                LineText[1] := '';
                LineText[2] := '';
                LineText[3] := '';
                ...
                //PAB
                if StrPos("Sales Invoice Header"."Sell-to Customer Name", 'ALSO') > 0 then begin
                    NormalizedPostingDate := NormalizeDate("Sales Invoice Header"."Posting Date");
                    NormalizedDueDate := NormalizeDate("Sales Invoice Header"."Due Date");
                    NormalizedShipmentDate := NormalizeDate("Sales Invoice Header"."Shipment Date");
                end;
                if StrPos("Sales Invoice Header"."Sell-to Customer Name", 'ALSO') = 0 then begin
                    NormalizedPostingDate := Format("Sales Invoice Header"."Posting Date");
                    NormalizedDueDate := Format("Sales Invoice Header"."Due Date");
                    NormalizedShipmentDate := Format("Sales Invoice Header"."Shipment Date");
                end;
                    ...
                    case ShipmentMethod."Read Incoterms City From" of
                        ShipmentMethod."read incoterms city from"::"Ship-to City":
                            begin
                                ...
                                ShipmentMethod.Description := StrSubstNo('%1 %2', "Sales Invoice Header"."Shipment Method Code", "Sales Invoice Header"."Ship-to City");  // 09-07-20 ZY-LD 001
                            end;
                ...
                //Check if reverse vat text should be inserted on footer
                LocalSalesInvoiceLine.SetRange("Document No.", "Sales Invoice Header"."No.");
                LocalSalesInvoiceLine.SetRange("Gen. Bus. Posting Group", 'FINANCE');
                LocalSalesInvoiceLine.Setrange("VAT Prod. Posting Group", 'REVERSE');
                If not LocalSalesInvoiceLine.isEmpty then
                    Footertext1 := 'Reverse Charge Rechnung. Steuerschuldnerschaft des Leistungsempfängers.';
    ...
    labels
    {
        ...
        IBANTxT = 'IBAN No.';
        Text001 = 'Rechnung';
        Text002 = 'Rechnungsadresse';
        Text003 = 'Lieferadresse';
        Text004 = 'Rechnungsnr.';
        Text005 = 'Belegdatum';
        Text006 = 'Kundenummer';
        Text007 = 'VAT Reg. No.';
    ...
    var
        ...
        Text005: label 'Rechnung';
        PageCaptionCap: label 'Page %1 of %2';
        Text006: label 'Total %1 Excl. VAT';
        Footertext1: Text[100];
