Report 50007 "Sales - Credit Memo RHQ"
{
    // 001. 31-10-17 ZY-LD Sell-to Country Region.
    // 002. 15-02-18 ZY-LD 2018021310000143 - CompanyInfo."VAT Registration No." is changed to CompVATRegNo.
    // 003. 13-08-18 ZY-LD 2018081010000496 - Company VAT Reg. No. was not visible. It's visible now.
    // 004. 11-02-19 ZY-LD 2019020810000044 - Commercial credit memo.
    // 005. 06-06-19 ZY-LD P0213 - New setup for VAT Registration No. Zyxel.
    // 006. 09-08-21 ZY-LD 2021070210000087 - Position on External document No.
    // 007. 31-05-22 ZY-LD 2022053010000106 - "External Document No." and "Tarriff No." is only shown on items.
    // 008. 10-06-22 ZY-LD 2022061010000099 - "EU 3-Party Trade".
    // 009. 26-04-23 ZY-LD 2679188 - Show VAT in local currency if currency is different from LCY.
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Sales - Credit Memo RHQ.rdlc';
        ...
        dataitem("Sales Cr.Memo Header"; "Sales Cr.Memo Header")
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(DocumentCaptionCopyText; StrSubstNo(DocumentCaption, CopyText, SalesOrCommercial))
                    {
                    }
                    ...
                    column(PostingDate_SalesCrmHdr; Format("Sales Cr.Memo Header"."Posting Date", 0, '<Closing><Day>. <Month Text> <Year4>'))
                    {
                    }
                    ...
                    column(DueDate_SalesInvHdr; Format("Sales Cr.Memo Header"."Due Date", 0, '<Closing><Day>. <Month Text> <Year4>'))
                    {
                    }
                    ...
                    column(DocDate_SalesCrmHdr; Format("Sales Cr.Memo Header"."Document Date", 0, '<Closing><Day>. <Month Text> <Year4>'))
                    {
                    }
                    ...
                    dataitem("Sales Cr.Memo Line"; "Sales Cr.Memo Line")
                    {
                        ...
                        column(CressRef_SalesInvLine; "Sales Cr.Memo Line"."Item Reference No.")
                        {
                        }
                        dataitem("Sales Shipment Buffer"; "Integer")
                        {
                            DataItemTableView = sorting(Number);
                            column(SalesShptBufferPostDate; Format(SalesShipmentBuffer."Posting Date", 0, '<Closing><Day>. <Month Text> <Year4>'))
                            {
                            }
                        ...
                        begin
                            ...
                            //>> 31-10-17 ZY-LD 001
                            if SellToCountryRegion."Show Country of Origin on Inv." then
                                if (recItem."Country/Region of Origin Code" <> '') and
                                    ItemCountryRegion.Get(recItem."Country/Region of Origin Code")
                                then
                                    TarrifCode := CopyStr(TarrifCode + ', COO: ' + ItemCountryRegion.Name, 1, MaxStrLen("Sales Cr.Memo Line".Description));
                            //<< 31-10-17 ZY-LD 001
                            ...
                            if "Sales Cr.Memo Line".Type in ["Sales Cr.Memo Line".Type::"G/L Account", "Sales Cr.Memo Line".Type::Item] then
                                ReportEvent.GetEUArticle(
                                  ...
                                  "Sales Cr.Memo Header"."EU 3-Party Trade",  // 10-06-22 ZY-LD 008
                                  ArticleText);
                        ...
                        begin
                            ...
                            if CalculateCurrency then begin
                                if ToCurrencyCode = '' then
                                    Error(zText003);
                                CalculatedVATAmount := recCurrExchRate.ExchangeAmount(VATAmountLine."VAT Amount", CurrencyCode, ToCurrencyCode, "Sales Cr.Memo Header"."Posting Date");
                            end else
                    ...
                    {
                        DataItemTableView = sorting(Number) where(Number = const(1));
                        column(NOK_VAT_HEADER; StrSubstNo(zText002, ToCurrencyCode, ExchRate))
                        {
                        }
            ...
            begin
                ...
                //>> 26-04-23 ZY-LD 009
                if recIntCompPurch.Get("Sales Cr.Memo Header"."Intercompany Purchase") then begin
                ...
                else  //<< 06-06-19 ZY-LD 005
                      //>> 15-02-18 ZY-LD 002
                    if "Sales Cr.Memo Header"."VAT Registration Code" <> '' then begin
                        VATRegistrationRec.Get("Sales Cr.Memo Header"."VAT Registration Code");
                ...
                if "Sales Cr.Memo Header"."Sell-to Country/Region Code" <> '' then
                    SellToCountryRegion.Get("Sales Cr.Memo Header"."Sell-to Country/Region Code");
                //<< 31-10-17 ZY-LD 001
    ...
    var
        ...
        Text011: label '%2 - Prepmt. Credit Memo %1';
        ...
        CompVATRegNo: Code[50];
        NormalizedPostingDate: Text[8];
        SalesOrCommercial: Text[10];
        ArticleText: Text;
        ...
        ExchRate: Decimal;
        zText002: label 'VAT Amount (%1): (Ex.Rate: %2)';
    ...
    begin
        //>> 11-02-19 ZY-LD 002
        BillToCountryRegion.Get("Sales Cr.Memo Header"."Bill-to Country/Region Code");
        if BillToCountryRegion."Commercial Invoice" then
            SalesOrCommercial := Text012
        else
            SalesOrCommercial := Text013;
        //<< 11-02-19 ZY-LD 002
