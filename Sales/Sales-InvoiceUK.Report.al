report 50016 "Sales - Invoice UK"
{
    ...
    // 003. 11-12-20 ZY-LD 2020121010000091 - Incoterms city can be different.
    // 004. 09-08-21 ZY-LD 2021070210000087 - Position on External document No.
    // 005. 21-01-22 ZY-LD 2022012010000089 - There can be bank account´s with different currency code, we therefore read from bank account instead of company info.
    ...
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Sales - Invoice UK.rdlc';
        ...
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(CompanyInfoBankName; recBankAcc.Name)
                    {
                    }
                    column(CompanyInfoBankAccNo; recBankAcc."Bank Account No.")
                    {
                    }
                    column(CompanyInfoBankAdress; recBankAcc.Address)
                    {
                    }
                    column(CompanyInfoBankPostCode; recBankAcc."Post Code")
                    {
                    }
                    column(CompanyInfoBankCity; recBankAcc.City)
                    {
                    }
                    column(CompanyInfoCurrCode; recBankAcc."Currency Code")
                    {
                    }
                    column(CompanyInfoIBAN; recBankAcc.Iban)
                    {
                    }
                    column(CompanyInfoSwift; recBankAcc."SWIFT Code")
                    {
                    }
            ...
            begin
                ...
                */
                ...
                if recBankAcc.Count() > 1 then begin
                    recBankAcc.SetRange("Print on Sales document", true);
                    recBankAcc.FindFirst();
                    if recBankAcc.Count() > 1 then
                        if "Sales Invoice Header"."Currency Code" = GLSetup."LCY Code" then
                            Error(zText001, StrSubstNo('%1|%2', "Sales Invoice Header"."Currency Code", ''))
                        else
                            Error(zText001, "Sales Invoice Header"."Currency Code");
                end;
                recBankAcc.TestField(Name);
    ...
    labels
    {
        ...
        ExternalDocument = 'Customer PO No.:'; //03-04-2025 #486528
        Tariff = 'Tariff No.:';
