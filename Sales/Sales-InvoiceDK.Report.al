Report 50014 "Sales - Invoice DK"
{
    // 001. 06-09-18 ZY-LD 2018090610000055 - Take VAT Registration No. from Location.
    // 002. 06-06-19 ZY-LD P0213 - New setup for VAT Registration No. Zyxel.
    // 003. 20-08-19 ZY-LD 2019081910000067 - "NOK_VAT" is moved to it's own DataItem, so it won't be blocked by ShowShippingAddr.
    // 004. 30-08-19 ZY-LD 2019083010000081 - Now showing NOK_VAT from ZyND NO.
    // 005. 18-10-19 ZY-LD 2019101810000055 - Located a bug in the code. Made other changes as well.
    // 006. 20-11-19 ZY-LD 2019121310000104 - Calculate NOK vat from USD to EUR to NOK.
    // 007. 07-01-20 ZY-LD 2020010710000063 - Reorganized the Norvegan VAT calculation
    // 008. 09-07-20 ZY-LD P0455 - Shipment Method caption is changed to Incoterms, and "Ship-to City is added".
    // 009. 11-12-20 ZY-LD 2020121010000091 - Incoterms city can be different.
    // 010. 09-08-21 ZY-LD 2021070210000087 - Position on External document No.
    // 011. 23-08-21 ZY-LD 000 - The VAT Registration No. on the customer can be different from the "VAT Registration No. (Sell-to)" on the invoice.
    // 012. 15-09-21 ZY-LD 2021091310000081 - Translate "Your Reference".
    // 013. 08-02-22 ZY-LD 2022012510000105 - Changed calculation of locat VAT.
    // 014. 31-05-22 ZY-LD 2022053010000106 - "External Document No." and "Tarriff No." is only shown on items.
    // 015. 10-06-22 ZY-LD 2022061010000099 - "EU 3-Party Trade".
    // 016. 12-02-24 ZY-LD 000 - Changed to Sell-to VAT.
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Sales - Invoice DK.rdlc';
        ...
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(HeaderExternalNo; "Sales Invoice Header"."External Document No.")
                    {
                    }
                    ...
                    column(SalesInvoiceHeaderIntercompanyPurchase; "Sales Invoice Header"."Intercompany Purchase")
                    {
                    }
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        column(Item_Reference_No_; "Sales Invoice Line"."Item Reference No.")
                        {
                        }
                        column(Item_Reference_No_Caption; "Sales Invoice Line".fieldCaption("Item Reference No."))
                        {
                        }
                        ...
                        begin
                            ...
                            if "Sales Invoice Line".Type in ["Sales Invoice Line".Type::"G/L Account", "Sales Invoice Line".Type::Item] then
                                ReportEvent.GetEUArticle(
                                  ...
                                  "Sales Invoice Header"."EU 3-Party Trade",  // 10-06-22 ZY-LD 015
                                  ArticleText);
                    ...
                    dataitem(VATCounter; "Integer")
                    {
                        ...
                        begin
                            ...
                            //>> 08-02-22 ZY-LD 013
                            if ((recIntCompPurch.Code <> '') and (recIntCompPurch."Calc. Local VAT for Currency" <> CurrencyCode) and (recIntCompPurch."Calc. Local VAT for Currency" <> '')) or
                               ((SalesSetup."Calc. Local VAT for Currency" <> CurrencyCode) and (SalesSetup."Calc. Local VAT for Currency" <> ''))
                            then begin
                                CalculateCurrency := true;
                                if recIntCompPurch."Calc. Local VAT for Currency" <> '' then
                                    ToCurrencyCode := recIntCompPurch."Calc. Local VAT for Currency"
                                else
                                    ToCurrencyCode := SalesSetup."Calc. Local VAT for Currency";
                            end;

                            /*IF ZGT.CompanyNameIs(10) THEN BEGIN  //  ZyND DK
                              IF "Sales Invoice Header"."Intercompany Purchase" = 'IC-NO' THEN
                            ...
                            if CalculateCurrency then begin
                                if ToCurrencyCode = '' then
                                    Error(ZText003);
                                CalculatedVATAmount := recCurrExchRate.ExchangeAmount(VATAmountLine."VAT Amount", CurrencyCode, ToCurrencyCode, "Sales Invoice Header"."Posting Date");
                                ...
                                IF CurrencyCode <> GLSetup."LCY Code" THEN BEGIN
                                  recCurrency.SETRANGE("Currency Code",CurrencyCode);
                                  recCurrency.SETFILTER("Starting Date",'..%1',"Sales Invoice Header"."Posting Date");
                                  IF recCurrency.FINDLAST THEN
                                    IF recCurrency."Relational Exch. Rate Amount" <> 0 THEN
                                      VatAmount := VatAmount * recCurrency."Relational Exch. Rate Amount" / recCurrency."Exchange Rate Amount";
                                END;
                                //<< 20-11-19 ZY-LD 006
                                recCurrency.SETRANGE("Currency Code",'NOK');
                                recCurrency.SETFILTER("Starting Date",'..%1',"Sales Invoice Header"."Posting Date");
                                IF recCurrency.FINDLAST THEN
                                  IF recCurrency."Relational Exch. Rate Amount" <> 0 THEN
                                    NOKVAT := VatAmount / recCurrency."Relational Exch. Rate Amount" * recCurrency."Exchange Rate Amount";*/
                            end else
                                CalculatedVATAmount := 0;  //VATAmountLine."VAT Amount";  // 18-10-19 ZY-LD 005
                            //<< 07-01-20 ZY-LD 007
                            //<< 08-02-22 ZY-LD 013
                    ...
                    dataitem(NOK_VAT_Integer; "Integer")
                    {
                        DataItemTableView = sorting(Number) where(Number = const(1));
                        column(NOK_VAT_HEADER; StrSubstNo(ZText001, ToCurrencyCode, ExchRate))
                        {
                        }
                        column(NOK_VAT; CalculatedVATAmount)
                        {
                        }
                    }
            ...
            begin
                    ...
                    case ShipmentMethod."Read Incoterms City From" of
                        ShipmentMethod."read incoterms city from"::"Ship-to City":
                            begin
                                ...
                                ShipmentMethod.Description := StrSubstNo('%1 %2', "Sales Invoice Header"."Shipment Method Code", "Sales Invoice Header"."Ship-to City");  // 09-07-20 ZY-LD 008
                            end;
                ...
                if ReferenceText <> '' then
                    if sellcust."Your Reference Translation" <> sellcust."your reference translation"::" " then
                        ReferenceText := Format(sellcust."Your Reference Translation");
                //<< 15-09-21 ZY-LD 012

                //RD001
                //SellVATID := sellcust."VAT Registration No.";  // 23-08-21 ZY-LD 011
                SellVATID := "Sales Invoice Header"."VAT Registration No.";  // 23-08-21 ZY-LD 011  // 12-02-24 ZY-LD 016
                if "Sales Invoice Header"."Ship-to VAT" <> '' then
                    SellVATID := "Sales Invoice Header"."Ship-to VAT";  // 12-02-24 ZY-LD 016
                LineText[1] := '';
                LineText[2] := '';
                LineText[3] := '';
                LineText[4] := '';
                ...
                //   VATRegistrationRec.GET("VAT Registration Code");
                ...
                else begin  //<< 06-06-19 ZY-LD 002
                            //recLocation.GET("Location Code");
                            //>> 01-10-18 ZY-LD 002
                            //recLocation.TESTFIELD("VAT Registration No");
                            //CompVATRegNo := recLocation."VAT Registration No";
                    if recLocation.Get("Sales Invoice Header"."Location Code") and (recLocation."VAT Registration No" <> '') then
                        CompVATRegNo := recLocation."VAT Registration No"
                    else begin
                        Error(ZText002);  // 31-08-21 ZY-LD 000
                                          //IF recVatRegPrCountry.GET("Sell-to Country/Region Code") THEN
                                          //  CompVATRegNo := recVatRegPrCountry."VAT Registration No.";
                    end;  //<< 01-10-18 ZY-LD 002
                          //<< 06-09-18 ZY-LD 001
                end;
    ...
    labels
    {
        ...
        ExternalDocument = 'Customer PO. No.:'; // 8-04-2025 BK #486528
        Tariff = 'Tariff No.:';
        PaymentFoot = 'Please provide payment according to the specific terms';
        PaymentFoot1 = 'Please transfer all payments to:';
        SwiftTxt = 'SWIFT Code:';
        IBANTxT = 'IBAN No.';
        Text001 = 'Reverse charge, article 28c A in Directive 77/388/EEC';
        Text002 = 'VAT in local currency (NOK):';
    }
    ...
    var
        ...
        Text021: label 'RHQ No.';
        DocumentTotals: Codeunit "Document Totals";
        ZText001: label 'VAT Amount (%1): (Ex.Rate: %2)';
        ...
        ZText002: label 'Send an e-mail to navsupport@zyxel.eu.';
        ...
        ZText003: label '"To Currency Code" must be filled to calculate local VAT.';
        ExchRate: Decimal;
