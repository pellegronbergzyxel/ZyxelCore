Report 50114 "Create Vend. Jnl. Lines OEntry"
{
    Caption = 'Create Vendor Journal Lines - Open Entries';
    ProcessingOnly = true;
    UsageCategory = Tasks;
    ...
    dataset
    {
        dataitem(Vendor; Vendor)
        {
            ...
            dataitem("Vendor Ledger Entry"; "Vendor Ledger Entry")
            {
                ...
                begin
                    ZGT2.UpdateProgressWindow(Format("Vendor Ledger Entry"."Posting Date"), 0, true);

                    if not VendorUnblocked then
                        if Vendor.Blocked <> Vendor.Blocked::" " then begin
                            VendorUnblocked := true;
                            VendTmp := Vendor;
                    ...
                    GenJnlLine.Validate("Document Date", "Vendor Ledger Entry"."Document Date");
                    GenJnlLine.Validate("Due Date", "Vendor Ledger Entry"."Due Date");
                    GenJnlLine.Validate("Document Type", "Vendor Ledger Entry"."Document Type");
                    GenJnlLine.Validate("Document Type", DocumentTypes);
                    GenJnlLine.Validate("Document No.", "Vendor Ledger Entry"."Document No.");
                    GenJnlLine.Validate("Account Type", GenJnlLine."account type"::Vendor);
                    GenJnlLine.Validate("Account No.", "Vendor Ledger Entry"."Vendor No.");
                    GenJnlLine.Validate(Description, Text005);
                    //GenJnlLine.VALIDATE("Dimension Set ID","Dimension Set ID");
                    GenJnlLine.Validate("Currency Code", "Vendor Ledger Entry"."Currency Code");
                    GenJnlLine.Validate(Amount, "Vendor Ledger Entry".Amount);
                    ...
                    then begin
                        ...
                        if "Vendor Ledger Entry"."Vendor Posting Group" <> '' then
                            if VendPostGrp.Get("Vendor Ledger Entry"."Vendor Posting Group") then begin
                                GenJnlLine.Validate("Bal. Account Type", GenJnlLine."bal. account type"::"G/L Account");
                                GenJnlLine.Validate("Bal. Account No.", VendPostGrp."Payables Account");
                            end;
                    ...
                    recDimSetEntry.SetRange("Dimension Set ID", "Vendor Ledger Entry"."Dimension Set ID");
                    ...
                    GenJnlLine.Validate(Description, StrSubstNo('Opening Balance (%1-%2)', "Vendor Ledger Entry"."Dimension Set ID", GenJnlLine."Dimension Set ID"));
                ...
                begin
                    ...
                    if VendorUnblocked then begin
                        VendorUnblocked := false;

                        Vendor.Blocked := VendTmp.Blocked;
                        Vendor.Modify;
                    end;
                end;
                ...
                begin
                    ZGT2.OpenProgressWindow('', "Vendor Ledger Entry".Count);
                end;
            }
    ...
    var
        ...
        VendorUnblocked: Boolean;
        ZGT: Codeunit "ZyXEL General Tools";
        ZGT2: Codeunit "ZyXEL General Tools";
        VendTmp: Record Vendor temporary;
