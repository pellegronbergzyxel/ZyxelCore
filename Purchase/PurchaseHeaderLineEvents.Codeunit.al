codeunit 50079 "Purchase Header/Line Events"
{
    ...
    Local procedure PurchaseLine_OnAfterValidateNoPurchaseLine(var PurchaseLine: Record "Purchase Line"; var xPurchaseLine: Record "Purchase Line"; var TempPurchaseLine: Record "Purchase Line" temporary; PurchaseHeader: Record "Purchase Header")
    var
        PurchHeader: Record "Purchase Header";
    begin
    ...
    begin
        if (Rec.Type in [Rec.Type::Item, Rec.Type::"G/L Account"]) and (Rec."No." <> '') and (Rec."Location Code" <> '') then begin
            ...
            if recAddPostGrpSetup.FindLast() then
    ...
    begin
        ...
        then begin
            ...
            if not PurchaseHeader."EShop Order Sent" and not PurchaseHeader.SentToAllIn then
                    ...
                    case recICPartner."Purchase Order Comm. Type" of
                        ...
                        recICPartner."purchase order comm. type"::"Web Service":
                            begin
                                ...
                                if ZyWebServMgt.SendPurchasOrders(copystr(recICPartner."Inbox Details", 1, 50), PurchaseHeader."No.", recICPartner."Customer No.") then begin
            ...
            if not PurchaseHeader.SentToAllIn and (recWarehouse.Warehouse > recWarehouse.Warehouse::" ") and (recVend."SBU Company" = recVend."sbu company"::" ") then begin
                Commit();
                if ZyWebServMgt.SendContainerDetails(copystr(CompanyName(), 1, 50), 2, PurchaseHeader."No.") then begin
    ...
    begin
        GetSourceDocInbound.CreateFromPurchOrderHideDialog(Rec);
    end;
    ...
    procedure DeleteWarehouseReceipt(var Rec: Record "Purchase Header")
    var
